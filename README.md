...
flowchart TD
    Api("HTTP API - Gin Framework")
        --> Service("Service Layer: Upload / Download / Resolve")

    Service --> Core("Internal Core Architecture")
    Core --> DAG("DAG Builder / CID Generator")
    Core --> Resolver("Resolver: Domain → CID")
    Core --> Storage("KV Storage: Local Metadata Store")

    Service --> Engine("Downloader Engine: HTTP / HTTPS / FTP / IPFS / CNS+Path")
    Engine --> FS("Local File System: Save / Compress / List")
```
